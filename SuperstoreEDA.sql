...
-- Revenue in every month and Year
select Month(Order_Date), OrderDateYear, round(sum(sales),2) TotalRevenue
from PortfolioProject..Superstore
--where state = 'California' and 
where OrderDateYear = 2018
group by OrderDateYear,month(Order_Date)
--order by month(Order_Date) asc
union
select Month(Order_Date), OrderDateYear, round(sum(sales),2) TotalRevenue
from PortfolioProject..Superstore
--where state = 'California' and 
where OrderDateYear = 2017
group by OrderDateYear,month(Order_Date)
--order by month(Order_Date) asc
union
select Month(Order_Date), OrderDateYear, round(sum(sales),2) TotalRevenue
...
from PortfolioProject..Superstore
--where state = 'California' and 
where OrderDateYear = 2015
group by OrderDateYear,month(Order_Date)
order by 2, 1 asc

-- Percentage of each customer segment
select Segment, count(segment) TotalSegments,  (select count(Customer_ID) from Superstore) TotalPurchases, 
cast((count(Segment)*1.0/(select count(Customer_ID) from Superstore)*100)as Decimal(4,2)) SegmentPercentage
from PortfolioProject..Superstore
group by Segment
order by 2 desc

-- Total sales in every state
select state, round(sum(sales),2) Revenue
from PortfolioProject..Superstore
group by State
order by 2 desc
